...
async def context_menu_emoji_add(interaction, message: discord.Message) -> None:
...
@app_commands.checks.has_permissions(manage_emojis_and_stickers=True)
@app_commands.checks.bot_has_permissions(manage_emojis_and_stickers=True)
async def context_menu_sticker_add(interaction, message: discord.Message) -> None:
...
class EmojiOpsCog(commands.Cog, name="Emoji Operations"):
    ...
    def __init__(self, client) -> None:
        self.client = client
        self.client.tree.add_command(context_menu_sticker_add)
        self.client.tree.add_command(context_menu_emoji_add)
    ...
    async def cog_unload(self) -> None:
        self.client.tree.remove_command(context_menu_sticker_add.name, type=context_menu_sticker_add.type)
        self.client.tree.remove_command(context_menu_sticker_add.name, type=context_menu_sticker_add.type)
    ...
    @staticmethod
    async def convert_str_to_emoji(ctx, entity: str) -> discord.Emoji | discord.PartialEmoji | None:
    ...
    async def sticker_info(self, ctx, sticker: str) -> None:
...
async def setup(client) -> None:
    ...
    await client.add_cog(EmojiOpsCog(client))
